...
import { readToken } from "@/lib/authenticate"; // Import the readToken function

export default function MainNav() {
  const [isExpanded, setIsExpanded] = useState(false); // Added state for the mobile menu
  const router = useRouter();
  const [searchField, setSearchField] = useState(''); // Added state for the search field
            ...
            {token ? ( // Check if the user is logged in (has token)
              <>
                <Form className="d-flex" onSubmit={(e) => {
                  e.preventDefault();
                  router.push(`/search?title=true&q=${searchField}`);
                }}>
                  <Form.Control
                    type="search"
                    placeholder="Search"
